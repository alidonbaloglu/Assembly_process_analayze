...
class TaskStartPointDetector:
    ...
    def process_video(self, video_path: str, output_path: str = None, display: bool = True, mask_path: Optional[str] = None) -> List[Dict]:
        ...
        # --- YENİ EKLENEN KODLAR ---
        FRAME_SKIP_RATE = 3  # Her 5 karede bir YOLO çalıştır (Bu değeri ayarlayın)
        last_task_transition = None # Son sonucu sakla
        # --- BİTTİ ---
        ...
        try:
            while True:
                ...
                # --- GÜNCELLENEN MANTIK ---
                task_transition = None # Bu frame için varsayılan
                
                if frame_count % FRAME_SKIP_RATE == 0:
                    # Sadece her N. karede algılamayı çalıştır
                    ...
                    last_task_transition = task_transition # Sonucu sakla
                else:
                    # Aradaki kareler için son bilinen sonucu kullan
                    task_transition = None 
                
                # 'task_transition' uyarısını göstermek için son saklanan değeri kullan
                # (Eğer 5 kare önce geçiş olduysa, sayaç hala çalışıyor olmalı)
                # NOT: Bu kısım, bir önceki cevaptaki 'transition_frames_remaining' 
                # sayacıyla zaten doğru çalışacaktır.
...
def main():
    ...
    YOLO_MODEL_PATH = "C:/Users/ali.donbaloglu/Desktop/Montaj_proces/Modeller/Tez/yolo11m_20260116_051021/weights/best.pt"  # YOLO model dosyasının yolu
    VIDEO_PATH = "C:/Users/ali.donbaloglu/Desktop/Montaj_proces/input_video/Part26_yeni.mp4"   # Video dosyasının yolu
    OUTPUT_VIDEO_PATH = "Part26_output_tspd.mp4"  # Çıkış video dosyası (opsiyonel)
    RESULTS_JSON_PATH = "Part26_tspd_results.json"  # Sonuçlar JSON dosyası
