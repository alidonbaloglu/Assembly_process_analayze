...
class TaskStartPointDetector:
    ...
    def detect_task_transition(self, frame_i: int, frame_fi: np.ndarray) -> Optional[Dict]:
        ...
        if detected_name is None:
            # Nesne kaybolduysa mevcut görevi bitir ve beklemeye geç
            if self.current_task_name is not None and not self.waiting:
                ...
                self.detection_log.append({
                    ...
                    'detection': None,
                    'info': None,
                    'event': 'task_end_object_lost',
                    'details': {'ended_object': previous_object}
                })
            return None
        ...
        if detected_norm != expected_name:
            ...
            # Kararlılık kontrolü: aynı uyumsuz tespiti ardışık yeterince frame görmeden uyarma
            ...
            self.detection_log.append({
                ...
                'detection': detected_name,
                'info': current_detection_info,
                'event': 'order_violation',
                'details': violation_record
            })

            # Görsel uyarı için bir süre overlay göster ve tekrar uyarı için cooldown başlat
            self.order_violation_message = warn_msg
            fps_val = getattr(self, 'video_fps', 30) or 30
            self.order_violation_frames_remaining = max(self.order_violation_frames_remaining, int(1 * fps_val))
            self.violation_cooldown_frames = int(2 * fps_val)
            return None
        
        # Beklenen görev tespit edildi
        if self.current_task_name is None:
            # Beklemeden sonra ya da ilk kez beklenen görev başlangıcı
            if self.waiting and self.last_task_index is not None:
                wait_frames = frame_i - (self.wait_start_frame or frame_i)
                # Son görevin bekleme bilgilerini ve bir sonraki nesnesini güncelle
                try:
                    fps_val = getattr(self, 'video_fps', 30) or 30
                    wait_seconds = float(wait_frames) / float(fps_val) if fps_val > 0 else 0.0
                    # Yalnızca eşik üzerindeyse beklemeyi kabul et (ilk 2 saniyeyi düş)
                    ...
                    if wait_seconds >= wait_min:
                        ...
                        # Bekleme periyodunu listeye ekle
            ...
            if self.n == 1:
                ...
                print(f"📍 Yeni görev başladı - Frame {frame_i}: {detected_name}")
...
def main():
    ...
    VIDEO_PATH = "C:/Users/ali.donbaloglu/Desktop/Montaj_proces/input_video/eski/part17.mp4"   # Video dosyasının yolu
